...
describe('KanbanCard', () => {
  describe('rendering', () => {
    it('renders the task title', () => {
      const wrapper = mount(KanbanCard, {
        props: { task: baseTask },
        global: { plugins: [i18n], stubs: { AppCheckbox: true, AppPill: true } }
      })
  ...
  describe('done/pending state', () => {
    ...
    it('checkbox receives done as modelValue', () => {
      ...
      const wrapper = mount(KanbanCard, {
        ...
        global: {
          ...
          stubs: {
            AppCheckbox: {
              template: '<div class="app-checkbox-stub" :data-checked="modelValue" />',
              props: ['modelValue']
            },
            AppPill: true
          }
      ...
      expect(wrapper.find('.app-checkbox-stub').attributes('data-checked')).toBe('true')
    })
    ...
    it('checkbox receives false modelValue when task not done', () => {
      ...
      expect(wrapper.find('.app-checkbox-stub').attributes('data-checked')).toBe('false')
    })
  })
  ...
  describe('tag pill', () => {
    it('renders a AppPill when task has a tag', () => {
    ...
    it('does not render a AppPill when task has no tag', () => {
      const wrapper = mount(KanbanCard, {
        props: { task: baseTask },
        global: {
          plugins: [i18n],
    ...
    it('does not render a AppPill when tag is empty string', () => {
