...
describe('AppIcon', () => {
  ...
  it('renders nothing for an unknown name', () => {
    const wrapper = mount(AppIcon, { props: { name: 'nope' } })
  ...
  it('renders a solid variant when solid prop is true', () => {
    ...
    const wrapper = mount(AppIcon, { props: { name: 'plus', solid: true } })
    expect(wrapper.find('svg').exists()).toBe(true)
  })
  ...
  it('logs a console.warn in dev mode for unknown icon names', () => {
    ...
    mount(AppIcon, { props: { name: 'does-not-exist' } })
    expect(warnSpy).toHaveBeenCalledWith('[AppIcon] Unknown icon name: "does-not-exist"')
  })

  it('does not render a solid icon for unknown name', () => {
    const wrapper = mount(AppIcon, { props: { name: 'does-not-exist', solid: true } })
    expect(wrapper.find('svg').exists()).toBe(false)
  })
})
