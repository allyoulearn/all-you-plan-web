  ...
  <div>
    ...
    <template v-else-if="project">
      ...
      <div class="grid grid-cols-3 gap-4">
        <!-- Progress card -->
        <Card>
          <p class="text-[11px] font-medium uppercase tracking-widest text-muted">Progress</p>
          <p class="font-serif text-[36px] font-normal leading-none text-ink">{{ percent }}%</p>
          <ProgressBar :value="percent / 100" />
        </Card>

        <!-- Done / open card -->
        <Card>
          <p class="text-[11px] font-medium uppercase tracking-widest text-muted">Done · open</p>
      ...
      <template v-for="section in sections" :key="section.key">
        ...
        <p v-else class="mt-1 text-[13px] text-muted">No tasks here.</p>
      </template>
